-# 更新日誌 (CHANGELOG) - GitHub Pages 版本

本檔案記錄 GitHub Pages 版本的所有重要變更。
...
## [v1.0.6-github] - 2026-02-10

### [新增]
- **加速機分頁進化需求顯示功能**
  - 新增進化路線分組顯示，每個進化目標清楚顯示所需物品
  - 支援物品組格式：`物品名稱;數量|物品名稱;數量`
  - 支援數量區間格式：`0~5` 或 `1-5`
  - 新增折疊/展開功能，預設為折疊狀態
  - 區塊標題為「進化需求」
...
### [變更]
- **加速機分頁資料結構調整**
  - 欄位 33-38 (AG-AL)：改為進化物品組 1-6
  - 欄位 39 (AM)：版本欄位（原本在欄位 37）

### [樣式]
- 新增進化需求區塊樣式（分組顯示、左側藍色邊框）
- 新增折疊按鈕動畫效果

### [修正]
- **修復 JSON 資料載入問題**
  - 修正 `autoLoadDefaultData` 函式以適應實際 JSON 格式
  - 修正資料類型處理，正確識別數字和字串
  - 跳過 null 值以模擬 XLSX 行為
  - 修復超超代、搖擺X、加速機分頁顯示 null 的問題
- **修復超連結失效問題**
  - 改用 cellAddress 格式讀取超連結（與 convert-to-json.js 一致）
  - 修復介紹分頁超連結功能

### [部署]
- 更新 JSON 資料檔案（10 個工作表，1541 列，150.02 KB）
- 新增 66 個圖檔

---

## [v1.0.5-github] - 2026-02-09
...
- **加速機分頁物品數量區間顯示**
  - 支援物品數量區間格式（例如：`meatm;1-5`）
  - 區間顯示格式：`x 1~5`
  - 適用於欄位 33-36（圖檔索引 1-4）

### [變更]
- `processDetailData()` 函式更新：加速機分頁使用與搖擺X相同的欄位結構
- `parseItemImage()` 函式新增區間格式解析
- `showDetailView()` 函式新增加速機分頁篩選支援

### [修正]
- 修復加速機分頁版本欄位讀取位置（從第 33 欄改為第 37 欄）
